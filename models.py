...
def chat_with_llm(user_message):
    ...
    """
    # Более строгая проверка на приветствие
    user_message_lower = user_message.lower().strip()
    greetings = ['привет', 'здравствуй', 'добрый день', 'доброе утро', 
                'добрый вечер', 'hello', 'hi', 'здравствуйте', 'доброго времени суток']
    
    # Проверяем, начинается ли сообщение с приветствия
    if any(user_message_lower.startswith(greeting) for greeting in greetings):
        return "Здравствуйте! Чем могу помочь?"
    
    # Для остальных сообщений используем LLM
    try:
